...
if criterion=="GD":
    ...
    ana_list = [0.05, 0.1,0.3]


Data_folder = os.getcwd()+ResultPath
results_files = [f for f in os.listdir(Data_folder) if os.path.isfile(os.path.join(Data_folder, f))]
...
for p in range(len(par_ind)):
    for es_i in range(len(ana_list)):
            ...
            for n_p in range(len(range_npp)):
                ...
                for file in results_files:
                    ...
                    if file.startswith(prefix_to_match) and file.endswith('.csv'):  # check prefix
                        ...
                        PowerFile_path = os.getcwd()+ResultPath+file


                        PowerResults = pd.read_csv(PowerFile_path, delimiter = ',')["power_"+p_list[p]].dropna(axis = 0)
                        conven_cor = pd.read_csv(PowerFile_path, delimiter = ',')['conventional_power'].dropna(axis = 0)

                        heatmap_p[n_t,n_p] = PowerResults.iloc[0]
                        heatmap_p_c[n_t,n_p] = conven_cor.iloc[0]
        ...
        for i_p in range(len(par_ind)):
            ...
            if c == 1:
                fig.suptitle("Conventional power with p-value threshold = 0.05 ".format(par_ind[p]), fontsize=fontsize)
                ax,figure = plot_MultiHeatmap(axs[es_i],es_i,len(ana_list),c_data,norm,title,xticklabels,yticklabels,xylabels)
            else:
