...
def MLE(fun,arg,param_bounds,method = "Differential_Evolution", show = 0, n_ini = 2):
    """
    ...
    if method=="Brute": 
        ...
        estimated_parameters = estimated_parameters_SinIni
    elif method=="Nelder-Mead_MulIni":
        estimated_parameters_MulIni = np.empty((n_ini * param_bounds.shape[1],param_bounds.shape[1]+1))
        for r in range(n_ini * num_par): # loop for multiple initial guesses
            start_params = np.random.uniform(param_bounds[0],param_bounds[1])
            if show :
                print("initial guess:",r)
                # print('start point',start_params,fun(start_params,arg))
        ...
        estimated_parameters_DE = optimize.differential_evolution(fun, ranges, args = (arg,)) 
        estimated_parameters = estimated_parameters_DE.x
        if show :
            print('estimation from DE',estimated_parameters,fun(estimated_parameters,arg)) 
